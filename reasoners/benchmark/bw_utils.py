...
def generate_all_actions(state):
    ...
    if "hand is empty" in state:
        ...
        block_color = []
        for b in block:
            m = re.search("the ([a-z]{0,10}) block is clear", b)
            if m:
                block_color.append(m.group(1))
        for c in block_color:
            ...
            if f"the {c} block is on the table" in state:
                ...
                m = re.search(f"the {c} block" + " is on top of the ([a-z]{0,10}) block", state)
                if m:
                    c_ = m.group(1)
                    return_list.append(f"unstack the {c} block from on top of the {c_} block")
    else:
        m = re.search("is holding the ([a-z]{0,10}) block", state)
        if m is None:
            m = re.search("hand is holding the ([a-z]{0,10}) block", state)
        if m is None:
            return return_list
        c = m.group(1)
        block = re.findall("the [a-z]{0,10} block is clear", state)
...
def apply_change(change, state):
        ...
        if c.startswith("the hand"):
            ...
            for idx in range(len(states)):
                if ("hand is " + old) in states[idx]:
                    ...
                    success += 1
                elif ("is " + old) in states[idx]:
                    # 兼容 "is holding the X block" 写法，统一成 "hand is empty"
                    states[idx] = "hand is " + new
                    success += 1
        else:
    ...
    for s in states:
        if "have that" in s:
            ...
            priority_states.append(1)
        elif "the hand is" in s or ("hand is" in s and ("empty" in s or "holding" in s)):
